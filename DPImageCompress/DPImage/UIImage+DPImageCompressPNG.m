...
@implementation UIImage (DPImageCompressPNG)
...
//    liq_set_quality(handle, 75, 90);//图片质量范围 > 0 < 100;
    return [self compressToPNG:outputFilePath withMiniMum:75 withTarget:90];
}
- (NSData *)compressToPNG:(NSString *)outputFilePath withMiniMum:(int)minimum withTarget:(int)target{
    ...
    liq_set_quality(handle, minimum, target);//图片质量范围 > 0 < 100;
...
//    liq_set_quality(handle, 75, 90);//图片质量范围 > 0 < 100;
    return [self compressToPNG:inputFilePath output:outputFilePath withMiniMum:75 withTarget:90];
}
